...
function insert_modelData(modeType) {
    ...
    $.ajax({
        ...
        dataType: "JSON",
        data: JSON.stringify({modType: modeType, modHtml: modHtml_v, dataJson: JSON.stringify(modelData)}),
        success: function (result) {
            if (result.code == 0) {
                alert("提交成功");
            }
...
//初始化模板数据列表
function init_modelDataTable() {
    $("#datagrid2").datagrid({
        border: false,
        fitColumns: true,
        singleSelect: true,
        url: home_url + mmdList,
        columns: [[
            {field: 'modId', title: '编号', width: 25},
            {field: 'modelName', title: '模板名称', width: 40},
            {field: 'roomName', title: '科室', width: 25},
            {field: 'createTime', title: '创建时间', width: 55},
            {
                field: 'opt', title: '打开', width: 50,
                formatter: function (value, rec) {
